...
Item {
    ...
    property real safePosition: justChanged ? 0 : (player?.playbackState === MprisPlaybackState.Stopped ? 0 : (player?.position ?? 0))
    
    Connections {
        target: root.player
        function onTrackTitleChanged() {
            root.justChanged = true;
            resetTimer.restart();
            root.player.positionChanged();
        }
        function onPositionChanged() {
            if (root.player && root.player.position < 3) {
                root.justChanged = false;
            }
        }
    }
    ...
    Timer {
        id: resetTimer
        interval: 1500
        onTriggered: root.justChanged = false
    }
    ...
    Rectangle {
        ...
        Loader {
            ...
            Component {
                ...
                PlayerControls {
                    player: root.player
                    safePosition: root.safePosition
