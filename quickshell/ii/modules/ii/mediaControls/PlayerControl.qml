...
Item { // Player instance
    ...
    implicitHeight: (root.lyricsData && root.lyricsData.length > 0) ? 410 : 160
    ...
    property string nextLyric: ""
        ...
        let cur = "♫";
        let nxt = root.lyricsData[0].text;
        
        for (let i = 0; i < root.lyricsData.length; i++) {
            if (root.lyricsData[i].time <= pos) {
                cur = root.lyricsData[i].text;
                nxt = (i + 1 < root.lyricsData.length) ? root.lyricsData[i+1].text : "";
            } else {
                break;
            }
        }
        root.currentLyric = cur;
        root.nextLyric = nxt;
    }
    Process {
        id: lyricsFetcher
        running: true
        command: ["python3", `${FileUtils.trimFileProtocol(Directories.scriptPath)}/get_lyrics.py`, root.player?.dbusName || "", root.player?.trackTitle || ""]
        stdout: SplitParser {
            onRead: data => {
                try {
                    root.lyricsData = JSON.parse(data);
                } catch(e) {
                    root.lyricsData = [];
                }
                console.log("Lyrics data length:", root.lyricsData.length); root.updateLyrics();
            }
        }
    }
    ...
    Rectangle { // Background
        ...
        ColumnLayout {
            ...
            ColumnLayout { // Info & controls
                ...
                Item { Layout.fillHeight: true; visible: root.currentLyric === "" }
            ...
            ListView {
                ...
                Layout.fillWidth: true
                Layout.fillHeight: true
                visible: root.lyricsData && root.lyricsData.length > 0
                clip: true
                ...
                delegate: StyledText {
                    ...
                    font.pixelSize: isCurrent ? Appearance.font.pixelSize.large : Appearance.font.pixelSize.normal
                    color: isCurrent ? blendedColors.colOnLayer0 : blendedColors.colSubtext
                    opacity: isCurrent ? 1.0 : (Math.abs(lyricsList.currentIndex - (index || 0)) < 3 ? 0.7 : 0.3)
                    text: modelData.text
                    
                    property bool isCurrent: lyricsList.currentIndex === index
